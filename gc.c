            ...
            switch (BUILTIN_TYPE(vp)) {
                default: /* majority case */
                    ...
                    if (obj_free(objspace, vp)) {
                        ...
                        // becuase when we compact we're populating the swept
                        // pages freelist, we want to make sure freed slots get
                        // added there regardless
                        (void)VALGRIND_MAKE_MEM_UNDEFINED((void*)p, BASE_SLOT_SIZE);
    ...
    while(!gc_compact_cursors_met(objspace)) {
        for (int i = 0; i < SIZE_POOL_COUNT; i++) {
            ...
            if (!gc_compact_page(objspace, size_pool, heap, start_page)) {
                ...
                // If we're hear the cursors must have met for this heap
...
gc_ref_update(void *vstart, void *vend, size_t stride, rb_objspace_t * objspace, struct heap_page *page)
{
    ...
    for (; v != (VALUE)vend; v += stride) {
        /* if (!MARKED_IN_BITMAP(GET_HEAP_MARK_BITS(v), v) && BUILTIN_TYPE(v) != T_NONE) { */
        /*     fprintf(stderr, "updating refs for unmarked object %s\n", obj_info(v)); */
        /*     continue; */
        /* } */
