...
// Codegen for getting an instance variable. Gets the instance variable on `self` when `ivar_call` is false.
// Makes a bunch of assumptions about the caller, so please keep the caller in mind while reading.
static codegen_status_t
gen_get_ivar(jitstate_t *jit, ctx_t *ctx, const int max_chain_depth, const bool ivar_call, const struct rb_callinfo *ci, const rb_callable_method_entry_t *cme)
{
    // Only compile call to attrs that go through
    if (ivar_call && vm_ci_argc(ci) != 0) {
    ...
    if (iv_index_tbl && rb_iv_index_tbl_lookup(iv_index_tbl, id, &ent)) {
        ...
        if (ivar_call) {
            // Pop the receiver into REG0
            mov(cb, REG0, ctx_stack_pop(ctx, 1));

            // The caller (gen_opt_send_without_block) guards that
            // comptime_val_klass is equal to runtime reciver class. That
            // combined with the allocator check above mean that the receiver
            // has to be a heap object.
        }
        else {
            // Guard that the receiver has the same class as the one from compile time.
            mov(cb, REG0, member_opnd(REG_CFP, rb_control_frame_t, self));
            guard_self_is_object(cb, REG0, side_exit, ctx);
            jit_guard_known_klass(jit, &starting_context, comptime_val_klass, REG0, max_chain_depth, side_exit);
        }
...
{
    return gen_get_ivar(jit, ctx, GETIVAR_MAX_DEPTH, false, NULL, NULL);
}
...
gen_opt_send_without_block(jitstate_t* jit, ctx_t* ctx)
{
    ...
    case VM_METHOD_TYPE_IVAR:
        return gen_get_ivar(jit, ctx, OSWB_MAX_DEPTH, true, ci, cme);
